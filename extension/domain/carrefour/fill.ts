...
const heldOf = (cart: Cart, ean: string): number =>
  cart.lines.find((line): boolean => line.ean === ean)?.counter ?? 0;

const shortfalls = (planned: ResolvedLine[], cart: Cart): string[] =>
  planned
    .filter((line): boolean => heldOf(cart, line.ean) !== line.units)
...
const sightingsOf = (planned: ResolvedLine[], cart: Cart): Sighting[] =>
  planned
...
export const createBasketFiller = (
  shop: ShopClient,
): {
  run: (planned: PlannedLine[], windows: SlotWindow[], report: Report) => Promise<FillResult>;
} => {
  const substituter = createSubstituter(shop);

  const emptyExisting = async (report: Report): Promise<void> => {
    const existing = await shop.cart();
  ...
  const substituteAll = async (
    ...
    for (const line of lines) {
      const match = await substituter.findFor(line);
      if (match === undefined) {
        giveUp.push(line);
        await report({
          kind: 'LINE_MISSING',
  ...
  const settle = async (planned: ResolvedLine[], report: Report): Promise<Cart> => {
    let cart = await shop.put(planned.map((line) => ({ ean: line.ean, counter: line.units })));
    ...
    for (let round = 0; round < CORRECTION_ROUNDS; round += 1) {
      ...
      cart = await shop.put(corrections);
    }

    for (const line of planned) {
      const held = heldOf(cart, line.ean);
      await report(
        held === line.units
  ...
  const book = async (windows: SlotWindow[], report: Report): Promise<void> => {
    const offered = await shop.slots();
    ...
    await shop.bookSlot(chosen.ref);
  ...
  const run = async (
    ...
    const session = await shop.session();
    if (!session.signedIn) {
      await report({
        kind: 'BLOCKED',
    ...
    await emptyExisting(report);

    const known = planned.filter(isResolved);
    const substituted = await substituteAll(
      planned.filter((line): boolean => !isResolved(line)),
      report,
    );
    const resolved = [...known, ...substituted.found];

    const filled = await settle(resolved, report);

    const missing = [
      ...shortfalls(resolved, filled),
    ...
    return {
      ...
      missing,
      sightings: sightingsOf(resolved, filled),
    };
  };
