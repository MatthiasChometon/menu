...
import type { PlannedLine, SearchHit, ShopClient, Substitution } from './type';
...
export const createSubstituter = (
  shop: ShopClient,
): { findFor: (line: PlannedLine) => Promise<Substitution | undefined> } => ({
  findFor: async (line: PlannedLine): Promise<Substitution | undefined> => {
    ...
    const hits = await shop.search(searched);

    return bestOf(hits, required, line.unitSize);
  },
});
